...
class NetG(nn.Module):
    ...
    def forward(self, noise, c, labels, eval=False):
        # 原有的生成逻辑
...
class NetD(nn.Module):
    def __init__(self, ndf, imsize, ch_size, mixed_precision, num_classes):
        super(NetD, self).__init__()
        self.mixed_precision = mixed_precision
        self.label_embedding = nn.Embedding(num_classes, 768) #self.label_embedding = nn.Linear(num_classes, 768) 
        self.reduce_dim_conv = nn.Conv2d(1536, 768, 1)  # 添加一个1x1卷积层用于降维 4.17
    ...
    def forward(self, h, labels):
        with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
            label_features = self.label_embedding(labels) #4.17
            label_features = label_features.unsqueeze(-1).unsqueeze(-1)  # 维度变为[batch_size, 768, 1, 1]
            label_features = label_features.expand(-1, -1, h.size(3), h.size(3))  # 扩展到[batch_size, 768, 7, 7] 4.17
            #print("h[:, 0] shape:", h[:, 0].shape)
            #print("label_features shape:", label_features.shape)
            out = torch.cat([h[:,0], label_features], dim=1)  # 将标签向量与图像特征拼接 #4.17
            out = self.reduce_dim_conv(out) # 4.17
            #out = h[:,0]
...
class Classifier(nn.Module):
    def __init__(self, input_size, text_feature_size, hidden_size, output_size):
        super(Classifier, self).__init__()
        self.fc1 = nn.Linear(input_size, hidden_size)
        self.fc2 = nn.Linear(hidden_size + text_feature_size*7*7+1, output_size)  #25089
        #self.fc2 = nn.Linear(hidden_size + text_feature_size + 1, output_size)
        self.relu = nn.ReLU()
        self.sigmoid = nn.Sigmoid()
        #self.num_classes = num_classes

    def forward(self, img_feats, text_feats, labels):  # Use labels as input
        ...
        #
        #print(f"img_feats device: {img_feats.device}")  # print img_feats device
        #print(f"text_feats device: {text_feats.device}")  # print text_feats device
        #print(f"labels device: {labels.device}")  # print labels device

        #print(f"Labels size: {labels.size()}")

        x = self.fc1(img_feats)
        x = self.relu(x)
        #labels = torch.tensor([labels])  # Convert scalars to tensors
        if labels.dtype != torch.float32:
          labels = labels.float()
        labels = torch.unsqueeze(labels, dim=1)  # Convert a scalar to a tensor of size [1, 1]
        
        #print(f"Labels size: {labels.size()}")
        labels = labels.to(device)


        # Stitching text features with image features and labels
        x = torch.cat((x, text_feats, labels), dim=1)  # Splice labels with image and text features
        x = self.fc2(x)
        x = self.sigmoid(x)
        logits = self.sigmoid(x)#4.14

        # Calculate the predicted probability for each category using the softmax function
        probs = F.softmax(x, dim=1)

        # Identify the category with the highest probability of prediction
        _, predicted_class = torch.max(probs, 1)
        #print(f"Labels size: {labels.size()}")

        return logits, probs, predicted_class#4.14
'''


class Classifier(nn.Module):#0419
    def __init__(self, input_size, text_feature_size, hidden_size, output_size):
        super(Classifier, self).__init__()
        self.fc1 = nn.Linear(input_size, hidden_size)
        self.fc2 = nn.Linear(hidden_size + text_feature_size*7*7+1, output_size)
        self.relu = nn.ReLU()

    def forward(self, img_feats, text_feats, labels):
        device = "cuda" if torch.cuda.is_available() else "cpu"
        x = self.fc1(img_feats)
        x = self.relu(x)
        labels = labels.float()
        ...
        labels = labels.to(device)
        x = torch.cat((x, text_feats, labels), dim=1)  # 连接图像特征、文本特征和标签
        logits = self.fc2(x)
        probs = F.softmax(logits, dim=1)  # 使用softmax计算概率分布
        _, predicted_class = torch.max(probs, dim=1)  # 获取最可能的类别
        return logits, probs, predicted_class
    


class NetC(nn.Module):
    def __init__(self, ndf, cond_dim, text_feature_size, hidden_size, output_size, mixed_precision):
        ...
        self.classifier = Classifier(input_size=ndf*7*7, text_feature_size=ndf, hidden_size=hidden_size, output_size=output_size)
        
        #self.classifier = Classifier(input_size=512, text_feature_size=512, hidden_size=256, output_size=output_size)
        

    def forward(self, out, cond, labels):
        cond = cond.view(-1, self.cond_dim, 1, 1)
        cond = cond.repeat(1, 1, 7, 7)#4.12
        #print("out shape:", out.shape)
        #print("cond shape:", cond.shape)
        h_c_code = torch.cat((out, cond), 1)
        out_discriminator = self.joint_conv(h_c_code)
        out_discriminator = torch.sigmoid(out_discriminator)

        desired_channels =64  #调整后的调消数
        out_resized=out[:,:desired_channels,:,:]#
        cond_resized =cond[:,:desired_channels,:,:]#
        # Get the original logits from the classifier to facilitate calculating the loss
        # Calling the classifier
        #out_classifier = self.classifier(out_resized.view(out_resized.size(0), -1), cond_resized.view(cond_resized.size(0), -1), labels)  # Output from the classifier
        logits, probs, predicted_classes = self.classifier(out_resized.view(out_resized.size(0), -1), cond_resized.view(cond_resized.size(0), -1), labels) 
        #return out_discriminator, out_classifier
        #ti'aprint("Logits shape:", logits.shape)
        print("Logits shape:", logits.shape)
        return  out_discriminator, logits, probs, predicted_classes
