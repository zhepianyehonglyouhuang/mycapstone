...
'''
#######0419 建议完整替换load_models_opt
def load_models_opt(netG, netD, netC, optim_G, optim_D, optim_C, path, multi_gpus):
    checkpoint = torch.load(path, map_location=torch.device('cpu'))
    netG = load_model_weights(netG, checkpoint['model']['netG'], multi_gpus)
    netD = load_model_weights(netD, checkpoint['model']['netD'], multi_gpus)
    netC = load_model_weights(netC, checkpoint['model']['netC'], multi_gpus)
    optim_G = load_opt_weights(optim_G, checkpoint['optimizers']['optimizer_G'])
    optim_D = load_opt_weights(optim_D, checkpoint['optimizers']['optimizer_D'])
    optim_C = load_opt_weights(optim_C, checkpoint['optimizers']['optimizer_C'])
    return netG, netD, netC, optim_G, optim_D, optim_C
...
def save_models_opt(netG, netD, netC, optG, optD, optC, epoch, multi_gpus, save_path): #0419 建议完整替换
    if (multi_gpus==True) and (get_rank() != 0):
        None
    else:
        state = {'model': {'netG': netG.state_dict(), 'netD': netD.state_dict(), 'netC': netC.state_dict()}, \
                'optimizers': {'optimizer_G': optG.state_dict(), 'optimizer_D': optD.state_dict(), 'optimizer_C': optC.state_dict()},\
                'epoch': epoch} 
        torch.save(state, '%s/state_epoch_%03d.pth' % (save_path, epoch))  #0419
...
def save_checkpoints(netG, netD, netC, optG, optD, optC, scaler_G, scaler_D, epoch, multi_gpus, save_path):# 0419 建议完整替换
    if (multi_gpus==True) and (get_rank() != 0):
        None
    else:
        state = {'model': {'netG': netG.state_dict(), 'netD': netD.state_dict(), 'netC': netC.state_dict()}, \
