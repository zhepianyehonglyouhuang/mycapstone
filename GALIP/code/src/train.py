...
from lib.modules import predict_loss
...
################################ 0419修改 main 修改点较多，建议完整替换
def main(args):
    # Initialize criterion for classification
    criterionC = nn.CrossEntropyLoss().to(args.device)  ##4.14
    #criterionD = nn.BCEWithLogitsLoss().to(args.device)  # Define discriminator loss function##4.14
    
    def criterionD(predictor, img_feature, text_feature, labels, negative): #0419
        output, err = predict_loss(predictor, img_feature, text_feature, labels, negative)
        return err
    ...
    CLIP4trn, CLIP4evl, image_encoder, text_encoder, netG, netD, netC, BERT = prepare_models(args)
    print('**************G_paras: ',params_count(netG))
    print('**************D_paras: ',params_count(netD)+params_count(netC))
    ###0331更新
    fixed_img, fixed_sent, fixed_words, fixed_z, fixed_labels = get_fix_data(train_dl, valid_dl, text_encoder, args) #0331更新
    if (args.multi_gpus==True) and (get_rank() != 0):
        None
    else:
    ...
    optimizerC = torch.optim.Adam(netC.parameters(), lr=args.lr_c, betas=(0.0, 0.9)) #0419
    ...
    if args.state_epoch!=1:
        ...
        netG, netD, netC, optimizerG, optimizerD, optimizerC = load_models_opt(netG, netD, netC, optimizerG, optimizerD, optimizerC, path, args.multi_gpus) #0419
    ...
    for epoch in range(start_epoch, args.max_epoch, 1):
        ...
        torch.cuda.empty_cache()
        train(train_dl, netG, netD, netC, text_encoder, image_encoder, optimizerG, optimizerD, optimizerC, scaler_G, scaler_D, BERT, args, criterionC, criterionD) #0419
        torch.cuda.empty_cache()
        # save
        if epoch%save_interval==0:
            save_models_opt(netG, netD, netC, optimizerG, optimizerD, optimizerC, epoch, args.multi_gpus, args.model_save_file)
            torch.cuda.empty_cache()#0419
        
        # sample 
        if epoch%gen_interval==0:
           #sample(fixed_z, fixed_sent, netG, args.multi_gpus, epoch, args.img_save_dir, writer)
           sample(fixed_z, fixed_sent, fixed_labels, netG, args.multi_gpus, epoch, args.img_save_dir, writer) ####0418hd替换
           torch.cuda.empty_cache()
...
if __name__ == "__main__":
    mp.set_start_method('spawn') ### 4.12 avoid deadlock(/usr/lib/python3.10/multiprocessing/popen_fork.py:66: RuntimeWarning: os.fork() was called. os.fork() is incompatible with multithreaded code, and JAX is multithreaded, so this will likely lead to a deadlock.
  #self.pid = os.fork())
